...
- **认证由平台全局注入，不要生成登录节点**（平台的认证 Token 配置会给每个请求自动带 Authorization 头）
- 业务数据前置（如"租户已存在""账户有余额"）需要生成前置节点：节点 id 用 step_pre_<n>，放在主业务节点之前；造数式前置必须配套 cleanup 节点
- 不生成参数验证、边界情况
...
**命名格式**：
- 有前置造数：\`E2E - {前置步骤} - {业务操作}\`（前置节点 id 用 step_pre_<n>，造数式前置需配套 cleanup）
- 无前置造数：\`E2E - {业务操作}\`
- 注意：登录/认证不算前置——平台自动注入 Authorization，不出现在用例里

**示例**：
- "E2E - 创建租户 - 创建超管"  ← 业务数据前置（先有租户才能建超管）
- "E2E - 订单创建流程"
- "E2E - 异常: 已存在的名称，重复创建订单"

**示例场景**：
- 用户："测试商品管理功能"
  → 生成 4 个正面用例：新增商品+清理、删除商品、修改商品+清理、查询商品（认证由平台全局注入，不画登录节点）
  → 生成异常反面用例

- 用户："创建超管账户"（业务上要求租户先存在）
  → 生成：E2E - 创建租户 - 创建超管+清理（step_pre_1: 创建租户 → step_1: 创建超管 → step_cleanup_1: 删除租户）
