...
/**
 * 接口参数约束信息（从 Swagger/OpenAPI schema 提取）
 * 本期先随接口一起存储，供后续 AI 生成异常/边界用例时使用
 */
export interface ApiParamConstraints {
  // 必填字段路径，如 ["body.username", "body.email", "query.page"]
  required?: string[];
  // 枚举约束：字段路径 → 允许的取值
  enums?: Record<string, Array<string | number>>;
  // 数值/长度范围约束：字段路径 → { min, max, minLength, maxLength }
